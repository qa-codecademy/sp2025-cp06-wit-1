...
const ProjectDetailsView = async ({ id }) => {
	...
	let filteredProjects = allProjectModels
		.filter(p => {
			const daysLeft =
				typeof p.getDaysLeft === 'function' ? p.getDaysLeft() : 999;
			const remaining =
				typeof p.getRemainingAmount === 'function' ? p.getRemainingAmount() : 0;
	...
	if (filteredProjects.length === 0) {
		fallbackMessage = `<p>${
			t.noUrgentProjects ||
			'Нема итни проекти. Прикажуваме проекти со најмалку донации.'
		}</p>`;
	...
	const html = renderProjectDetails(
		project,
		type,
		t,
		fallbackMessage,
	...
	return {
		...
		setup: () => {
			...
			const overlay = document.querySelector(
				'#donationFormContainer .modal-overlay'
			);
			...
			copyLinkBtn?.addEventListener('click', e => {
				e.preventDefault();
				const url = `${window.location.origin}index.html#/project/${id}`;
			...
			const animateProgressBar = () => {
				...
				if (progress) {
					...
					const target = parseInt(
						progress.getAttribute('data-collected') || progress.value
					);
